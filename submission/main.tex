...
inside ADMM proximal iterations). Both are evaluated on identical
$y$'s as the diffusion methods. A DDRM reimplementation was attempted
but did not reach competitive PSNR in the available time.
...
declared at $p<0.05$ after Bonferroni correction.
...
Table~\ref{tab:main_nfe100} summarizes per-cell PSNR on the matched
grids at NFE${=}100$. On Gaussian blur, Pixel-DPS dominates FlowDPS-on-RF
in every cell by $2.39$--$4.20$\,dB (paired $t$-test
$p<10^{-17}$, Cohen's $d$ between $1.98$ and $3.52$, all
Bonferroni-significant). On matched motion blur, the same ordering
holds with an even wider gap of $+3.95$\,dB mean across 12 cells, all
significant at $p<10^{-22}$. The DPS-vs-FlowDPS-RF ordering is
therefore not specific to Gaussian; it persists across a second
forward-operator family with at least as large effect sizes.
Per-cell tables for both grids appear in
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    \toprule
    Forward operator & Wiener & DnCNN$+$PnP & FlowDPS-on-RF & Pixel-DPS \\
    \midrule
    Gaussian $\sigma_b{=}1.5$ & 24.28 & 24.61 & 23.65 & \textbf{26.21} \\
    Gaussian $\sigma_b{=}3.0$ & 24.03 & 24.31 & 22.58 & \textbf{26.25} \\
    Gaussian $\sigma_b{=}5.0$ & 22.50 & 22.81 & 21.13 & \textbf{25.33} \\
    \midrule
    Motion $L{=}15, \theta{=}0$  & --- & --- & 22.54 & \textbf{25.83} \\
    Motion $L{=}25, \theta{=}45$ & --- & --- & 21.35 & \textbf{25.38} \\
    Motion $L{=}35, \theta{=}45$ & --- & --- & 20.46 & \textbf{24.79} \\
    \midrule
    s/img on RTX 3060 & 0.02 & 0.30 & 23.5 & 17.9 \\
    \bottomrule
  \end{tabular}
\end{table}
...
untested. (iii)~Two matched forward-operator families (Gaussian + motion);
...
available time; we use DnCNN$+$PnP-ADMM as the analogous classical-prior
reference. (v)~All headline numbers for FlowDPS-on-RF use the non-EMA
...
classes (super-resolution, inpainting) are natural next steps that
